DROP FUNCTION IF EXISTS gp_toolkit.gp_unzip_partition_name(p_schema_name varchar, p_table_name varchar, p_part_name varchar, p_tablespace varchar);
DROP FUNCTION IF EXISTS gp_toolkit.gp_unzip_partition(p_schema_name varchar, p_table_name varchar, p_part_name varchar, p_tablespace varchar);

CREATE OR REPLACE FUNCTION gp_toolkit.gp_unzip_partition(p_schema_name varchar, p_table_name varchar, p_part_name varchar, p_tablespace varchar default 'pg_default'::varchar)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
...
BEGIN
    ...
    END IF;
    raise notice '% INFO start gp_toolkit.gp_unzip_partition_name() with params: %, %, %, %', timeofday()::timestamp, schema_name, table_name, part_name, part_tablespace;
    ...
    IF NOT FOUND THEN
        raise notice '% ERROR table partition name NOT FOUND', timeofday()::timestamp;
        RETURN;
    END IF;
    ...
    v_sql:= 'create table ' || tmp_table_name || ' (like "' || schema_name || '"."' || table_name || '"' ||
        ' including defaults including constraints) TABLESPACE ' || part_tablespace || ';';
    raise notice '% INFO %', timeofday()::timestamp, v_sql;
	execute v_sql;
	...
	execute v_sql;
    -- Success
    raise notice '% INFO end gp_toolkit.gp_unzip_partition_name() success', timeofday()::timestamp;
    RETURN;
EXCEPTION
    when others then
