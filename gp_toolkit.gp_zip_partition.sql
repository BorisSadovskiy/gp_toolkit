DROP FUNCTION IF EXISTS gp_toolkit.gp_zip_partition(p_schema_name varchar, p_table_name varchar, p_part_rank int8, p_tablespace varchar);

CREATE OR REPLACE FUNCTION gp_toolkit.gp_zip_partition(p_schema_name varchar, p_table_name varchar, p_part_rank int8, p_tablespace varchar default 'pg_default'::varchar)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
...
BEGIN
    ...
    IF (coalesce(schema_name,'') = '') or (coalesce(table_name,'') = '') or (coalesce(p_part_rank,0) <= 0) THEN
        raise notice '% ERROR some parameters are empty: schema_name="%", table_name="%", part_rank="%"', timeofday()::timestamp, schema_name, table_name, p_part_rank::varchar(30);
        RETURN;
    END IF;
    ...
    END IF;
    raise notice '% INFO start gp_toolkit.gp_zip_partition_name() with params: %, %, %, %', timeofday()::timestamp, schema_name, table_name, p_part_rank::varchar(30), p_tablespace;
    -- Get table partition name
    select pp.partitiontablename into table_part_name from pg_partitions pp 
    where pp.schemaname = schema_name and pp.tablename = table_name 
        and pp.partitionrank = p_part_rank;
    IF NOT FOUND THEN
        raise notice '% ERROR table partition NOT FOUND: schema_name="%", table_name="%", part_rank="%"', timeofday()::timestamp, schema_name, table_name, p_part_rank::varchar(30);
        RETURN;
    END IF;
    ...
    v_sql:= 'alter table "' || schema_name || '"."' || table_name || '"' ||
        ' EXCHANGE PARTITION FOR (RANK('|| p_part_rank::varchar(30) ||')) WITH TABLE ' || tmp_table_name || ' WITH VALIDATION;';
    raise notice '% INFO %', timeofday()::timestamp, v_sql;
	execute v_sql;
